...
def initialize_graph() -> None:
    ...
    try:
        ...
        logging.exception(f"Falha na inicialização do grafo: {e}")
    ...
    if st.session_state.relatorio_analise:
        ...
        if relatorio.detalhes_evolutiva:
            ...
            col1.metric(
                ...
                value=f"{relatorio.detalhes_evolutiva.estimativa_pontos_funcao} PF",
            )
            col2.metric(
                label="Prazo (Dias Úteis)",
                value=f"{relatorio.detalhes_evolutiva.prazo_dias_uteis} dias",
            )
            col3.metric(
