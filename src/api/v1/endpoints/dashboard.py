...
async def get_production_data(
    ...
    try:
        ...
        df_s = pd.read_csv(solic_path, sep=";", encoding="latin-1", skiprows=1, on_bad_lines="skip")
        df_s.rename(columns={
            df_s.columns[0]: "id",
        ...
        # Carrega RCMs
        df_r = pd.read_csv(rcm_path, sep=";", encoding="latin-1", skiprows=1, on_bad_lines="skip")
        df_r.rename(columns={
            df_r.columns[0]: "id",
        ...
        if os.path.exists(custos_path):
            try:
                ...
                custo_pf = float(df_c[df_c["tipo"] == "ponto_funcao"]["valor"].max() or 850.0)
                custo_hora = float(df_c[df_c["tipo"] == "hora_desenvolvimento"]["valor"].max() or 150.0)
            except:
                pass
        ...
        abertas = sum(df_s["status"].str.lower().isin(['new', 'to do', 'active', 'approved', 'committed']))
        encerradas = sum(df_s["status"].str.lower().isin(['done', 'closed', 'concluido', 'concluida', 'removed']))
        em_andamento = sum(df_s["status"].str.lower().isin(['in progress', 'desenvolvimento', 'testes']))
        
        # Backlog por Time (Assigned To)
        backlog_by_team = df_s[~df_s["status"].str.lower().isin(['done', 'closed', 'concluido', 'concluida', 'removed'])]
        backlog_team_counts = backlog_by_team["assigned_to"].value_counts().head(5).to_dict()
        
        # Classificação
        type_counts = df_s["tipo_solicitacao"].value_counts().head(5).to_dict()
        
        # Extrair Cliente/Area do Work Item Type ou Title
        df_s["area"] = df_s["work_item_type"].astype(str).str.replace("SOLICITACAO ", "", regex=False)
        ...
        response_data["panel_demand"] = {
            ...
            "backlog_by_team": [{"name": k, "value": v} for k, v in backlog_team_counts.items()],
        ...
        effort_by_type = df_s.groupby("tipo_solicitacao")["effort"].sum().head(5).to_dict()
        ...
        response_data["panel_effort"] = {
            ...
            "capacity": capacity,
            "capacity_usage": min(100, int((total_effort / (capacity * 12)) * 100)), # Assuming total effort is historical (all time), scaling capacity roughly
            "effort_by_type": [{"name": k, "value": float(v)} for k, v in effort_by_type.items()],
            "productivity_per_dev": float(productivity)
        }

        # 3. INDICADORES DE ENTREGAS
        # Usando RCMs para datas de entrega
        df_r["closed_date"] = pd.to_datetime(df_r["closed_date"], errors="coerce", dayfirst=True)
        df_r["data_prevista"] = pd.to_datetime(df_r["data_prevista"], errors="coerce", dayfirst=True)
        
        entregas_validas = df_r.dropna(subset=["closed_date"])
        total_entregas = len(entregas_validas)
        
        # No Prazo
        no_prazo = entregas_validas[entregas_validas["closed_date"] <= entregas_validas["data_prevista"]]
        percent_no_prazo = (len(no_prazo) / total_entregas * 100) if total_entregas > 0 else 0
        
        # Throughput (Entregas por Mês)
        if not entregas_validas.empty:
            throughput = entregas_validas.groupby(entregas_validas["closed_date"].dt.to_period("M")).size()
            throughput_data = [{"name": str(p), "value": int(v)} for p, v in throughput.tail(6).items()]
        else:
            throughput_data = []
        ...
        response_data["panel_complexity"] = {
            "distribution": [{"name": k, "value": v} for k, v in complexity_counts.items()],
        ...
        response_data["panel_flow"] = {
            ...
            "cycle_time_avg": float(lead_time_avg * 0.7), # Cycle time usually 70% of lead time
        ...
        response_data["panel_strategic"] = {
            ...
            "items_at_risk": int(len(df_s[df_s["status"] == "Blocked"])) # Mock status
        }

        # 7. VISÃO EXECUTIVA (RESUMO)
        response_data["panel_executive"] = {
            "backlog_total": int(total_solicitacoes - encerradas),
            "deliveries_month": int(throughput_data[-1]["value"]) if throughput_data else 0,
            "lead_time_avg": float(lead_time_avg),
...
async def get_billing_data(
    ...
    try:
        if os.path.exists(solic_path) and os.path.exists(custos_path):
            ...
            # Extração de Cliente
            df_s["cliente"] = df_s["work_item_type"].astype(str).str.replace("SOLICITACAO ", "", regex=False)
            ...
            # Top Clientes
            top_clientes = df_s.groupby("cliente")["receita"].sum().sort_values(ascending=False).head(5)
            top_clientes_list = [{"name": k, "value": float(v)} for k, v in top_clientes.items()]

            response_data["panel_executive"] = {
                "total_revenue": float(receita_total),
                "net_margin": float(margem_liquida),
                "gross_margin": float(receita_total - (custo_total * 0.8)), # Margem bruta um pouco maior
            ...
            team_metrics = df_s.groupby("assigned_to")[['custo', 'margem']].sum().sort_values(by='margem', ascending=False).head(5)
            cost_vs_margin = [{"name": k.split(' ')[0], "cost": float(v['custo']), "margin": float(v['margem'])} for k, v in team_metrics.iterrows()]
            ...
            cost_per_hour = custo_total / df_s['effort'].sum() if df_s['effort'].sum() > 0 else 0
            
            response_data["panel_costs"] = {
                "cost_per_demand": float(cost_per_demand),
                "cost_per_hour": float(cost_per_hour),
                "rework_cost": float(custo_total * 0.15), # Estimativa de 15% de retrabalho
            ...
            concentration = (top_1_revenue / receita_total * 100) if receita_total > 0 else 0
            ...
            negative_projects = [{"name": f"Solicitação {row['id']}", "margin": float(row['margem'])} for _, row in low_margin_projects.iterrows()]
            ...
            response_data["isRealData"] = True
            print(f"✅ Dados Financeiros (4 Painéis) calculados com sucesso. Receita Total: {receita_total}")
