...
async def execute_sprint_end_audit(start_date: datetime, end_date: datetime):
    ...
    bugs_found = sum(
        1 for i in closed_issues if "bug" in [l.lower() for l in i.get("labels", [])]
    )
...
async def execute_generic_audit(audit_type: str, details: dict):
...
async def generate_quarterly_report(start_date: datetime, end_date: datetime):
