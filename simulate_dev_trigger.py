...
# Payload de exemplo simulando o evento de LABEL ADICIONADA
# Isso deve disparar o Agente Desenvolvedor
payload_data = {
    "action": "labeled",
    "issue": {
        "number": 1003,
        ...
        "labels": [{"name": "status:pronto-para-dev"}],
    },
    "label": {"name": "status:pronto-para-dev"},
...
print(f"🔄 Enviando simulação de GATILHO DA FÁBRICA para: {API_URL}")
print(f"📦 Payload: Label='{payload_data['label']['name']}'")
...
try:
    ...
    if response.status_code == 202:
        print(
            "\n✅ SUCESSO: Webhook aceito! O Agente Desenvolvedor deve estar rodando em background."
        )
        print(
